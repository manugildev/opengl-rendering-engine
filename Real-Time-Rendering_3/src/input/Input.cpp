...
void Input::key_callback(int key, int scancode, int action, int mode) {
	...
	if (keys[GLFW_KEY_1]) {
		bool value = !app->get_component<GameObject>("barrel")->get_apply_normal_map();
		app->get_component<GameObject>("barrel")->set_apply_normal_map(value);
		app->get_component<GameObject>("rock")->set_apply_normal_map(value);
	}
	if (keys[GLFW_KEY_2]) {
		bool value = !app->get_component<GameObject>("rock")->get_show_normal_texture();
		app->get_component<GameObject>("rock")->set_show_normal_texture(value);
		app->get_component<GameObject>("barrel")->set_show_normal_texture(value);
	}
}
